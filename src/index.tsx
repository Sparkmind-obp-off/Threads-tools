...
function setupContent(): string {
  ...
  <section class="panel" aria-labelledby="sparkpod-daytona-title"><div class="panel-heading"><div><p class="eyebrow">SparkPod foundation</p><h2 id="sparkpod-daytona-title">Daytona Connector</h2></div><span class="badge neutral">Server-side</span></div><p>Enter the Daytona API key here. It is encrypted before storage in D1 and is never returned to the browser or committed to GitHub.</p><form method="post" action="/api/sparkpod/daytona/credentials" autocomplete="off"><label for="daytona-api-key">Daytona API key</label><input id="daytona-api-key" name="apiKey" type="password" minlength="20" maxlength="1024" autocomplete="new-password" required><div class="configuration-reference"><div class="reference-row"><span>Target</span><select name="target"><option value="us">US</option><option value="eu">EU</option></select></div><div class="reference-row"><span>API URL</span><input name="apiUrl" value="https://app.daytona.io/api" type="url" inputmode="url"></div></div><div class="actions"><button class="button primary" type="submit">Connect Daytona</button></div></form><div class="actions"><form method="post" action="/api/sparkpod/daytona/test"><button class="button secondary" type="submit">Test Connection</button></form><form method="post" action="/api/sparkpod/daytona/disconnect"><button class="button danger" type="submit">Disconnect</button></form></div><p class="muted-text">Test flow: create isolated Daytona sandbox → execute <code>printf "SparkPod OK"</code> → delete sandbox.</p></section>
