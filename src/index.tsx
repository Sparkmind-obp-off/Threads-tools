...
function setupContent(): string {
  ...
  <section id="sparkpod" class="panel sparkpod-panel" aria-labelledby="sparkpod-title">
    ...
    <div class="sparkpod-flow" aria-label="SparkPod connection test flow"><strong>Test flow</strong><span>Create sandbox</span><i aria-hidden="true">→</i><span>Wait until ready</span><i aria-hidden="true">→</i><span>Execute command</span><i aria-hidden="true">→</i><span>Verify result</span><i aria-hidden="true">→</i><span>Delete sandbox</span></div>
    ...
    <aside class="capability-note" aria-labelledby="daytona-preflight-title"><strong id="daytona-preflight-title">Daytona Reachability Diagnostic</strong><p>Checks the authenticated Daytona API endpoint without creating a sandbox. Reachability is diagnostic only and never counts as Connected.</p><div class="actions"><button id="daytona-preflight" class="button ghost" type="button">Run Reachability Diagnostic</button></div><div id="daytona-preflight-result" class="sparkpod-test-result hidden" aria-live="polite"></div></aside>
  </section>
